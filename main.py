...
# القالب الثابت (مأخوذ من طلب حقيقي)
...
def proxy_login():
    try:
        ...
        if response.status_code == 200:
            ...
            return jsonify({
                'success': True,
                'auth_token': response.content.hex(),
            ...
            return jsonify({
                'success': False,
                'error': f'Snapchat returned status {response.status_code}'
            }), 400
...
if __name__ == '__main__':
    print("[*] Starting Snapchat Login Proxy Server...")
    print("[*] Endpoint: /proxy_login")
