# server.py - ثبته على VPS (DigitalOcean, Linode, أو أي استضافة)
...
def login():
    try:
        ...
        # إدخال username/password من المستخدم
        decoded['1'] = username.encode()
        decoded['4'] = password.encode()
        typedef['4']['type'] = 'bytes'
        
        # إعادة التشفير
        re_encoded = blackboxprotobuf.encode_message(decoded, typedef)
        
        # بناء طلب gRPC
        grpc_body = struct.pack('>BI', 0, len(re_encoded)) + re_encoded
        ...
        if response.status_code == 200:
            return jsonify({'success': True, 'auth_token': response.content.hex()})
        else:
            return jsonify({'success': False, 'error': f'Status {response.status_code}'}), 400
